...
class Trainer():
    ...
    def set_model(self):
        with torch.no_grad():
            if self.pipeline == "LENet":
                ...
                # #SR model
                self.sr_model = DU_denoiser(ResNet_denoiser(1, channels = [8, 16, 32]), 3)
                weights_path = '/home/chranagno/repos/RangeSeg/model_SR.pth'
    ...
    def set_optim_scheduler(self):
        ...
        # all_params = list(self.model.parameters()) + list(self.sr_model.parameters())
        all_params = list(self.sr_model.parameters())
        ...
        if self.ARCH["train"]["scheduler"] == "consine":
            ...
            if self.ARCH["train"]["optimizer"] == "sgd":
                ...
                self.optimizer = optim.AdamW(
                    all_params, lr=self.ARCH["train"]["warmup"]["lr"])
